...
module.exports = function restfulQuery(tables, req, res, errorHandler){
    ...
    this.build = function(type, objOrg){
        ...
        if(typeof objParams._elastic === 'undefined'){
    ...
    this.$getTableIsElastic = function(obj){
        return self.tables[obj.table.name].elastic;
    };
    ...
    this.$getBoolean = function(obj,name){
        return (obj[name]==='true' || obj[name]===true);
    };
    ...
    this.$validateSort = function (obj,fields){ //validate sort fields
        ...
        for(var i=0;i<lngSort;i++){
            ...
            if(lngAryField>1){
                console.log('$validateSort: '+obj.namespace + '.' + aryField[0]);
    ...
    this.$validateAggregate = function (obj,fields){ //validate aggregate field
        ...
        for(var i=0;i<lngAggregate;i++){
            ...
            var lngAryField = aryField.length;
            if(lngAryField>1){
                console.log('$validateAggregate: '+obj.namespace + '.' + aryField[0]);
                if(typeof fields[obj.namespace + '.' + aryField[0]] !== 'undefined'){
                    ...
                    switch(objColumn.define.type){
                        case 'jsonb':
                            ...
                            obj.aggregates[i].name = obj.namespace + '.' + aryField[0];
                            obj.aggregates[i].keyName = keyName;
                            fieldValid = true;
                            if(typeof objColumn.actions.object !== 'undefined'){
                    ...
                    obj.aggregates[i].name = field;
                    fieldValid = true;
                }
            }else{
                if(typeof fields[obj.namespace + '.' + field]!=='undefined'){
                    obj.aggregates[i].name = obj.namespace + '.' + field;
                    fieldValid = true;
                }
            }
            if(fieldValid===false){
                return self.onError(400,100,obj._sectionExt+'aggregate field:\''+field+'\' was not found in the collection');
            }
