            ...
            try {
                ...
                $sql.connect().then(connection => {
                    connection.transaction(arySQL).then(() => {
                        ...
                        console.log(err);
                        connection.release();
                        self.errorHandler.error(400, err.code, err.detail + '');
            ...
            if (type === 'read') {
                switch ((task.type).toLowerCase()) {
                    case 'get':
                        ...
                        restfulGet.build(tables, task).then(resolve,reject);
                        break;
                    case 'list':
                        var restfulList = new $restfulList(req, res);
                        restfulList.build(tables, task).then(resolve,reject);
                        break;
                }
            } else {
                ...
                switch ((task.type).toLowerCase()) {
                    case 'insert':
                        ...
                        restfulInsert.build(tables, task, {transaction: true}).then(resolve,reject);
                        break;
                    case 'update':
                    case 'delete':
                        var restfulUpdate = new $restfulUpdate(req, res);
                        restfulUpdate.build(tables, task, {transaction: true}).then(resolve,reject);
                        break;
                    default:
