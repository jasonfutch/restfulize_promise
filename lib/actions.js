...
module.exports = function restfulActions(tables, req, res, errorHandler){
        ...
        return new $promise((resolve, reject) => {
            try {
                ...
                */
                                        ...
                                        $async.forEachOfSeries(objBuild.body, function(v,item,callback) {
                                            ...
                                            if (typeof column !== 'undefined') {
                                                ...
                                                if (column.define.type === 'jsonb') {
                                                    ...
                                                    if (objBuild.orgData !== null) {
                                                        if((typeof column.actions.objectArray!=='undefined' && column.actions.objectArray!=='') || (column.actions.allowedEmptyArray && column.actions.validator.indexOf("jsonb")===-1) || (column.actions.allowUpdatingWithPartialObject===false)) {
                                                ...
                                                if (value === false) {
                                                    ...
                                                    self.$restfulRoutine.onFieldAction(objBuild, obj, item).then(function(){
                                                         ...
                                                         * checkExistIn
                                                         * */
                                                        if (column.actions.checkExistIn.length > 0 && obj._skipCheckExistIn.indexOf(item)===-1) {
                                                ...
                                                for (var i = 0; i < lngFields; i++) {
                                                    ...
                                                    if (type === 'post') {
                                                        if (column.actions.required === true) {
                                                            if (column.actions.editable === false || column.actions.allowGenerated === true || column.actions.default !== '') {
                 ...
                 * */
                 if (type === 'put') {
                    self.$getOriginalData(obj).then(function (orgData) {
                        obj.orgData = orgData;
    ...
    this.$getOriginalData = function(obj){
        const restfulRequest = new $restfulRequest(tables, req, res, errorHandler);
        ...
        return new $promise((resolve, reject) => {
            ...
            if (obj._skipExistCheck) {
                ...
                const objTemp = restfulRequest.emptyObject();
                ...
                $sql.connect().then(function (connection) {
                    connection.execute(strSQL).then((rows) => {
                        if (rows.length > 0) {
                            connection.release();
                            resolve(self.formatResponse(obj, rows[0], true));
                        } else {
                            ...
                            connection.execute(strSQL).then(function (rows) {
                                connection.release();
                                if (rows.length > 0) {
                                    self.onError(403, 100, 'This '+obj.table+' record is forbidden.');
                                } else {
                                    self.onError(404, 100, obj.table+' record was not found.');
                                }
                                reject();
                            }, function (err) {
                                connection.release();
                                self.onError(400, err.code, err + '');
                                reject();
                            });
                        }
                    }, (err) => {
    ...
    this.$processFields = function(table, column, value, field, bolStrict, type, force, bolPrivate, objName){ //bolPrivate means the data is coming from internal and assumed safe
        ...
        if(column.actions.blankToNull===true){
