...
var $setData = function(dirName,aryDataFiles,bolForceRun){
    ...
    if(!aryDataFiles.logCodes) aryDataFiles.logCodes = [];
    ...
    var dirLogCode = $path.join(dirName, './data/logCodes/');
    ...
    if(prevDir!==dir || bolForceRun) {
        ...
        dirSchemas.forEach(function (file) {
            try {
                ...
                tables[fN].lists['blank'] = [];

                if(!tables[fN].elastic) tables[fN].elastic = false;
        ...
        var dirLogCodes;
        try {
            var dirlocalLogCodes = $fs.readdirSync(dirLogCode);

            var localLogCodes = [];
            dirlocalLogCodes.forEach(function (file) {
                localLogCodes.push(dirLogCode + file.replace(/\.[^/.]+$/, ""));
            });

            dirLogCodes = aryDataFiles.logCodes.concat(localLogCodes);
        }catch(e){
            dirLogCodes = aryDataFiles.logCodes;
        }

        dirLogCodes.forEach(function (file) {
            try {
                var aryFileName = file.split(/\\|\//);
                var fN = aryFileName[aryFileName.length-1];
                var fileLogCodes = require(file);

                /*
                 * Setting field defaults
                 * */
                var lngLogCodes = fileLogCodes.codes.length;
                for (var i = 0; i < lngLogCodes; i++) {
                    var logCode = fileLogCodes.codes[i];

                    logCode.scope = fN;

                    objLogCodes[logCode.code+""] = logCode;
                }
            } catch (e) {
                console.log(e);
            }
        });
        exports.logCodes = objLogCodes;

        exports.logCode = function(code){
            let objLog = {"code":0, "message":"Unrecognized Log Code.", "scope":"unknown"};

            if(typeof exports.logCodes[code+""] !== 'undefined'){
                objLog = exports.logCodes[code+""];
            }

            if(typeof objLog.type === 'undefined' || objLog.type === ''){
                objLog.type = 'event';
            }
            if(typeof objLog.level === 'undefined' || objLog.level === ''){
                objLog.level = 'action';
            }
            if(typeof objLog.action === 'undefined' || objLog.action === ''){
                objLog.action = 'notice';
            }
            if(typeof objLog.tags === 'undefined' || !Array.isArray(objLog.tags)){
                objLog.tags = [];
            }
            if(!objLog.timeline){
                objLog.timeline = false;
            }
            if(!objLog.merge){
                objLog.merge = false;
            }
        ...
        exports.errorCode = function(code,details,appendMessage){
            let error = {"code":code, "message":"Unrecognized Error Code.", "scope":"unknown"};
            let status = 400;
            if(typeof exports.errorCodes[code+""] !== 'undefined'){
                error = $extend(true,{},exports.errorCodes[code+""]);
            }
            ...
            if(typeof error.status !== 'undefined' && error.status !== '' && Number.isInteger(error.status)){
            ...
            //remove status from sent object
            delete error.status;
